...
#define POSITION_SCALE_FACTOR 100
#define MOTOR_COUNT 4U
#define CHASSIS_MODE_COUNT 3U

// PID参数常量
#define POSITION_DEADZONE 5
...
// 射击PID相关变量 (保留原有功能)
// =============================================================================
static const float Kp_shoot = 0.9f;
static const float Ki_shoot = 0.00097f;
static const float Kd_shoot = 0.8f;
...
static int32_t last_position_scaled[MOTOR_COUNT] = {0};   // 缩放后位置
...
static void update_motor_position(uint8_t motor_id)
{
    ...
    // 转换为缩放位置 (避免浮点运算)
    last_position_scaled[motor_id] = position_scaled[motor_id];
...
static int16_t calculate_pid(PID *pid, int32_t actual_val, int32_t integral_limit, int16_t deadzone)
{
    ...
    // 死区检查
    if (abs_int16(pid->err) < deadzone) {
        return 0;
    }
    if (abs_int16(pid->err) > 1000) {
    	return ((pid->err > 0)?3000:-3000);
    }
    ...
    // PID计算 (使用整数运算提高效率)
    int32_t p_term = (int32_t)(pid->Kp * pid->err);
    if(abs_int16(pid->err)<100){p_term *= addition();}
    int32_t i_term = (int32_t)(pid->Ki * pid->integral);
    int32_t d_term = (int32_t)(pid->Kd * (pid->err - pid->err_last));
 ...
 * @return 目标速度(RPM)
 */
static int16_t calculate_position_pid(PID *pid, int32_t actual_val, int32_t max)
{
    int16_t velocity = calculate_pid(pid, actual_val, INTEGRAL_LIMIT_POSITION, POSITION_DEADZONE);
    return clamp_int16(velocity, -max, max);
}
...
static void analyze_chassis_motion(void)
{
    ...
    for (uint8_t i = 0; i < MOTOR_COUNT; i++) {
        delta_pos[i] = position_scaled[i] - last_position_scaled[i];
    }
    
    // 计算底盘运动分量 (麦克纳姆轮运动学)
    int16_t turn = 		+ delta_pos[0] + delta_pos[1] + delta_pos[2] + delta_pos[3];        // 转向
    int16_t forward = 	- delta_pos[0] + delta_pos[1] + delta_pos[2] - delta_pos[3];        // 右移
    int16_t right = 	- delta_pos[0] - delta_pos[1] + delta_pos[2] + delta_pos[3];        // 前进
    
    // 更新位置积分
    chassis_position[0] += turn ;
    chassis_position[1] += right ;
    chassis_position[2] += forward ;
    
    pid_total_part[0] = chassis_position[0]/4;
    pid_total_part[1] = chassis_position[1]/4;
    pid_total_part[2] = chassis_position[2]/4;
    
    pid_location[0].target_val = pid_location[1].target_val = pid_location[2].target_val = 0;
    ...
    for (uint8_t i = 0; i < CHASSIS_MODE_COUNT; i++) {
    	if(i == chassis_mode){max_vol = 3000;}else{max_vol = 500;}
        chassis_velocity[i] = calculate_position_pid(&pid_location[i], pid_total_part[i],max_vol);
        pid_veloc_part[i] = chassis_velocity[i];
    }
    
    // 计算各轮目标速度 (麦克纳姆轮逆运动学)
    int16_t target_velocity[MOTOR_COUNT];
    target_velocity[0] = - chassis_velocity[2] - chassis_velocity[1] + chassis_velocity[0];
    target_velocity[1] = + chassis_velocity[2] - chassis_velocity[1] + chassis_velocity[0];
    target_velocity[2] = + chassis_velocity[2] + chassis_velocity[1] + chassis_velocity[0];
    target_velocity[3] = - chassis_velocity[2] + chassis_velocity[1] + chassis_velocity[0];
...
void pid_angle_init(void)
{
    ...
    for (uint8_t i = 0; i < MOTOR_COUNT; i++) {
        ...
        pid_position[i].Kp = 5.0f;
        pid_position[i].Ki = 0.01f;
        ...
        pid_velocity[i].Kp = 5.0f;
        pid_velocity[i].Ki = 0.5f;
    ...
    for (uint8_t i = 0; i < CHASSIS_MODE_COUNT; i++) {
        ...
        pid_location[i].Kp = 5.0f;
        pid_location[i].Ki = 0.001f;
...
void pid_angle(void)
{
    ...
    if (control_mode == 1) {
        ...
        // 位置控制模式
        for (uint8_t i = 0; i < MOTOR_COUNT; i++) {
            int16_t target_velocity = calculate_position_pid(&pid_position[i], position_scaled[i],3000);
...
int8_t check_pid(void)
{
    for (uint8_t i = 0; i < MOTOR_COUNT; i++) {
		...
		position_error = (control_mode)? pid_location[i].err : pid_position[i].err;
        if (abs_int16((int16_t)position_error) > POSITION_DEADZONE) {
            return 0;
        }
