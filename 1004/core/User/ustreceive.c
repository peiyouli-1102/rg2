...
static const char FISH_PREFIX[] = "fishing";
static const char SHOOT_PREFIX[] = "shoot";
...
static const uint32_t FISH_PREFIX_LEN = 7;          // "fishing"的长度
static const uint32_t SHOOT_PREFIX_LEN = 5;         // "shoot"的长度
...
int32_t extract_number(const uint8_t *buf, uint32_t buf_size, prefix_type_t *prefix_type) {
    ...
    result = try_match_prefix(SHOOT_PREFIX, SHOOT_PREFIX_LEN,
                             char_buf, buf_size, &success);

    if (success) {
        *prefix_type = PREFIX_SHOOT;
        return result;
    }
    ...
    result = try_match_prefix(FISH_PREFIX, FISH_PREFIX_LEN,
                             char_buf, buf_size, &success);

    if (success) {
        *prefix_type = PREFIX_FISH;
        return result;
    }
